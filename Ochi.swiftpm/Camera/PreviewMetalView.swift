...
class PreviewMetalView: MTKView {
    ...
    func configureMetal() {
        let defaultLibrary = try! device!.makeLibrary(source: ShaderLibrary.all, options: nil)
    ...
    override func draw(_ rect: CGRect) {
        ...
        CVMetalTextureCacheCreateTextureFromImage(kCFAllocatorDefault,
                                                  ...
                                                  0,
                                                  &cvTextureOut)
        guard let cvTexture = cvTextureOut, var texture = CVMetalTextureGetTexture(cvTexture) else {
        ...
        if blurValue > 0 {
            let blur = MPSImageGaussianBlur(device: device!, sigma: blurValue)
            blur.edgeMode = .mirror
